...
data class Book(
    ...
    fun getUseReplaceRule(defaultReplaceEnabled: Boolean): Boolean {
        ...
        //图片类书源 epub本地 默认关闭净化
        val isImage = type and BookType.image > 0
        val isLocal = if (type == 0) {
            origin == BookType.localTag || origin.startsWith(BookType.webDavTag)
        } else {
            type and BookType.local > 0
        }
        val isEpub = isLocal && originName.endsWith(".epub", true)
        if (isImage || isEpub) {
            return false
        }
    ...
    fun getPageAnim(): Int {
        var pageAnim = config.pageAnim
            ?: if (type and BookType.image > 0) {
                ...
                BookModelRuntimeRegistry.runtime.defaultPageAnimation()
            }
        if (pageAnim < 0) {
            pageAnim = BookModelRuntimeRegistry.runtime.defaultPageAnimation()
        }
        return pageAnim
    }
    ...
    fun getFolderName(): String {
        ...
        //防止书名过长,只取9位
        folderName = BookModelRuntimeRegistry.runtime.folderName(this)
        return folderName!!
    }
    ...
    fun migrateTo(
        ...
        return BookModelRuntimeRegistry.runtime.migrate(
            source = this,
            target = newBook,
            toc = toc,
            defaultReplaceEnabled = defaultReplaceEnabled,
