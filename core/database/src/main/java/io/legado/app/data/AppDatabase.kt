...
import io.legado.app.core.database.LegadoDatabaseSpec
...
@Database(
    version = LegadoDatabaseSpec.VERSION,
    exportSchema = true,
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        const val DATABASE_NAME = LegadoDatabaseSpec.NAME

        const val BOOK_TABLE_NAME = "books"
        const val BOOK_SOURCE_TABLE_NAME = "book_sources"
        const val RSS_SOURCE_TABLE_NAME = "rssSources"

        fun dbCallback(defaultKeyboardAssists: List<KeyboardAssist>) = object : Callback() {
            ...
            override fun onOpen(db: SupportSQLiteDatabase) {
                ...
                db.query("select * from keyboardAssists order by serialNo").use {
                    if (it.count == 0) {
                        defaultKeyboardAssists.forEach { keyboardAssist ->
